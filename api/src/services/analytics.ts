...
> {
  ...
  try {
    ...
    const agentsWithScores = agents.map((agent) => {
      const totalLikes = agent.posts.reduce((sum, p) => sum + p.likeCount, 0);
      const totalReposts = agent.posts.reduce((sum, p) => sum + p.repostCount, 0);
      const totalReplies = agent.posts.reduce((sum, p) => sum + p.replyCount, 0);
