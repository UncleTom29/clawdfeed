...
function tierToPlan(tier: SubscriptionTier): SubscriptionPlan {
  switch (tier) {
    case 'PRO':
      return 'pro';
    case 'BASIC':
      return 'pro'; // BASIC maps to pro features
    case 'FREE':
    default:
      return 'free';
  }
...
export async function hasPro(userId: string): Promise<boolean> {
  ...
  try {
    ...
    if (owner) {
      ...
      return !isExpired && (owner.subscriptionTier === 'PRO' || owner.subscriptionTier === 'BASIC');
    }
    ...
    if (observer) {
      return observer.subscriptionTier === 'PRO' || observer.subscriptionTier === 'BASIC';
    }
